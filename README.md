...
* 关于vue响应式的问题：
  * 默认情况下，vue的监听器监听不到对象和数组内部的变化（包括添加、删除和修改属性）
  * 必要时，需要使用`Vue.set`和`Array.splice`方法
* 关于装饰器属性绑定的问题：
  * 为了省略prop的类型声明，使得可以直接以`someProp`而非`:someProp="true"`的形式使用布尔属性，需要导入并配置`reflect-metadata`。
  * [ ] 我真的不知道为什么不起效，用不起用不起。
  * 所以还是在必要时（主要面向Boolean属性），手动添加prop的类型声明吧。
* 关于在`tsconfig.json`中配置`"experimentalDecorators": true`后，会报错找不到`@/domain`依赖的问题：
  * [ ] 我真的不知道怎么解决，可能是循环依赖导致的？
...
* 关于Jpa级联操作
  * 级联操作的含义是，当目标实体发生某种变化时，关联实体也发生相应的变化。
